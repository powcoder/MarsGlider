...
PLOT_PARTICLES = True # False for grading 
...
#Note for Mac OS High Sierra users having problems with "an error occured while attempting to obtain endpoint for listener" errors:
		...
		#Calculate the actual position of the target next timestep.
                ...
                if VERBOSE == True:
                   ...
                   if extra_points != None and len(extra_points) > 0:
                      ...
                      pAvg = sum(particle_dist) / len(particle_dist)
...
# sim.simulate_without_steering(marsglider.estimate_next_pos, param)
sim.simulate_with_steering(marsglider.next_angle, param)
